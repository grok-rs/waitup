...
async fn main() -> Result<(), wait_for::WaitForError> {
    ...
    let custom_status_target = vec![
        Target::parse("https://httpbin.org/status/204", 204)?,
    ];

    match wait_for_connection(&custom_status_target, &basic_config).await {
        Ok(result) => println!("✅ Custom status check (204) passed in {:?}", result.elapsed),
    ...
    let auth_headers = vec![
        ("Authorization".to_string(), "Bearer your-token-here".to_string()),
    ...
    match wait_for_connection(&multiple_targets, &any_config).await {
        Ok(result) => {
            ...
            println!("📊 Successful target: {}",
                     result.target_results.iter()
                           .find(|r| r.success)
    ...
    for target_result in &result.target_results {
        println!("  - {}: {} ({} attempts, {:?})",
                 ...
                 if target_result.success { "✅ Ready" } else { "❌ Failed" },
