...
impl Target {
    pub fn parse(target_str: &str, headers: &[Header]) -> Result<Self> {
        ...
        let (host, port_str) = target_str
            .split_once(':')
            .ok_or_else(|| Error::Config(format!("Invalid target '{target_str}': expected host:port or URL")))?;
        ...
        if port == 0 {
            return Err(Error::Config(format!("Port must be 1-65535, got 0 in '{target_str}'")));
        }
...
fn validate_headers(headers: &[Header]) -> Result<()> {
    for (key, value) in headers {
        ...
        if value.is_empty() {
            return Err(Error::Config("HTTP header value cannot be empty".to_string()));
        }
        if !key
            .chars()
